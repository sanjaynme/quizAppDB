...
public class GKDataSource {
    ...
    public void insertOrUpdateGkQuestion(ApiObject question) {
        long id = question.getId();
        Cursor c = database.rawQuery("SELECT * FROM " + QuizDBHelper.TABLE_NAME_GK+ " WHERE " + QuizDBHelper.COLUMN_ITEM_ID + "=" + id, null);
        if (c.getCount() == 0) {
            ...
            database.insert(QuizDBHelper.TABLE_NAME_GK, null, values);
        } else {
            //else update the row
            ...
            database.update(QuizDBHelper.TABLE_NAME_GK, updatedValues, QuizDBHelper.COLUMN_ITEM_ID + "=" + id, null);
        }
    }
