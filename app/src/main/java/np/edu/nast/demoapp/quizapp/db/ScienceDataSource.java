...
public class ScienceDataSource {
    ...
    public ScienceDataSource(Context context) {
    ...
    public void insertOrUpdateScienceQuestion(ApiObject question) {
        long id = question.getId();
        Cursor c = database.rawQuery("SELECT * FROM " + QuizDBHelper.TABLE_NAME_SCIENCE + " WHERE " + QuizDBHelper.COLUMN_ITEM_ID + "=" + id, null);
        if (c.getCount() == 0) {
            ...
            database.insert(QuizDBHelper.TABLE_NAME_SCIENCE, null, values);
        } else {
            //else update the row
            ...
            database.update(QuizDBHelper.TABLE_NAME_SCIENCE, updatedValues, QuizDBHelper.COLUMN_ITEM_ID + "=" + id, null);
        }
    }
    ...
    public List<ApiObject> getData() {
        ...
        Cursor cursor = database.query(QuizDBHelper.TABLE_NAME_SCIENCE, columns, null, null, null, null, null);
